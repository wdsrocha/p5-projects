...
function setup() {
  ...
  for (var i = 0; i < CITYMAX; i++) {
    const margin = 2 * VERTEX_STROKE_WEIGHT;
    var x = random(margin, width - margin);
    var y = random(margin, height / 2 - margin);
...
function draw() {
  ...
  stroke(0);
  strokeWeight(VERTEX_STROKE_WEIGHT);
  ...
  if (generation > MAXGEN) {
    ...
    for (var i = 1; i < cities.length; i++) {
      if (
        permutation[i - 1] == ansPath[i - 1] &&
        permutation[i] == ansPath[i]
      ) {
        stroke(0, 256, 0);
      } else {
        stroke(256, 0, 0);
      ...
      line(
        cities[ansPath[i - 1]].x,
        cities[ansPath[i - 1]].y,
        cities[ansPath[i]].x,
        cities[ansPath[i]].y
      );
...
function DP(src) {
  for (var i = 0; i < CITYMAX; i++) {
    memo[i] = [];
    for (var j = 0; j < 1 << CITYMAX; j++) memo[i][j] = -1;
  }
